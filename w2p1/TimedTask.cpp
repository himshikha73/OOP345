...
namespace sdds
{
	...
	void TimedTask::addTask(const char* taskName) {
		...
		//m_tasks->m_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(m_stopTime - m_startTime);
		//m_tasks->m_name = taskName;
		//m_tasks->m_timeUnits = "nanoseconds";
		//strcpy(m_tasks->m_name.c_str(), taskName);
	}
